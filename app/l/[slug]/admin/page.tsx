...
export default function LeagueAdminPage() {
  ...
  async function load() {
    ...
    setMembers(list);
    setJoinCode(lg.league.join_code);
    setLoading(false);
  }
  ...
  if (!lg.isAdmin) {
    return (
      <div className="max-w-md mx-auto text-center pt-10">
        ...
        <p className="text-ink/60 mb-6">You're not an admin of this league.</p>
  ...
  async function removeMember(userId: string) {
    if (!lg.league) return;
    if (!confirm('Remove this member from the league? Their historical scores will remain.')) return;
    const { error } = await supabase.from('league_members')
      .delete()
      .eq('league_id', lg.league.id)
      .eq('user_id', userId);
  ...
  async function regenerateCode() {
    ...
    const { error: updErr } = await supabase.from('leagues').update({ join_code: newCode }).eq('id', lg.league.id);
  ...
  return (
    <div className="space-y-12">
      ...
      <section className="tile-border p-6">
        ...
        <p className="text-xs text-ink/50 italic">Share this with new players. They enter it at <code>/leagues/join</code>.</p>
      </section>
