...
-- save_game_result (migration 0035): transactional score save/clear.
-- Replaces the client's delete→insert→update dance over PostgREST, which
-- could destroy the previous result on mid-save network failure (completed
-- game with zero rows reads as a Wall) and silently failed the delete for
-- non-admin members editing (DELETE RLS is admin-only boilerplate). Any
-- club member may record results; the playing set is derived server-side
-- (table_seats ∩ game_player_winds where not sitting out).
create or replace function public.save_game_result(p_game_id uuid, p_outcome text, p_winner_player_id uuid default null, p_points int default 0)
  returns void language plpgsql security definer set search_path to 'public', 'pg_catalog'
as $$
declare
  v_club_id uuid;
  v_table_id uuid;
begin
  -- Resolve + lock the game row (serializes concurrent saves/clears).
  ...
  -- Atomic wipe-and-rewrite: any failure rolls the whole thing back.
  delete from game_scores where game_id = p_game_id;
...
grant execute on function public.save_game_result(uuid, text, uuid, int) to authenticated, service_role;
